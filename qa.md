...
- Затем вернуть батарейку на место и не закрывая крышку корпуса вынуть модули оперативной памяти из слотов и затем установить их на место. (Возможно потерялся контакт, из за чего ПК не грузится).
...
- При нормальной загрузке ПК Диодны должны загораться в следующем порядке - CPU=>DRAM=>VGA=>Boot (Затем Boot может оставаться гореть, или все диодны должны гаснуть).