...
2. При открытии Boot menu выберете загрузку с USB накопителя Sandisk (64Gb) Partition 1, на которую установлена
   Clonezilla
...
- По умолчанию сервер может быть настроен на вывод изображения с видеокарты Intel UHD Graphics которая интегрирована в
  процессор. Она выводит изображение только на цифровые видеовыходы сервера. (HDMI, DVI-I, DisplayPort.)
...
5. Положение - 2-3 (- Выключено) - Используется дискретная видеокарта или встроенная в ЦП (Порты - HDMI, DVI-I,
   DisplayPort).
...
1. Для поиска битых секторов можно использовать утилиту badblocks. Если вам надо проверить корневой или домашний раздел
   диска, то лучше загрузится в LiveCD, чтобы файловая система не была смонтирована. Все остальные разделы можно
   сканировать в вашей установленной системе. Вам может понадобиться посмотреть какие разделы есть на диске. Для этого
   можно воспользоваться командой fdisk:
...
2. Остальные порты на сервере при этом работать не будут, т.к. за вывод изображения на них отвечает видеокарта
   встроенная в ЦП - Intel UHD Graphics 630.
...
1. Рекомендуется
   скачать [Crystal disk mark](https://sourceforge.net/projects/crystaldiskmark/files/8.0.5/CrystalDiskMark8_0_5.zip/download)
2. Сделать замеры скорости ввода/вывода Raid-массива. Нужно чтобы ничего параллельно не велась запись, иначе результаты
   будут неверными
3. После получения результат. Если результаты на поток на запись будет выше чем выдает RAID, то проблема в настройках
   Интеллект - нужно корректировать настройки, чтобы RAID справлялся
4. Также рекомендуется на время тестов отключить Firewall и антивирус т к они могут существенно влиять на фильтрацию
   сетевого трафика
...
6. При загрузки, Intellect X указать, его в качестве архивного для записи и проверить, как будет записываться и
   считываться архив
...
Рекомендую проверить настройки гибернации в ОС - возможно сервер переходит в нее после некоторого времени и не может
корректно выйти.
...
1. Если ОС загружается нормально, но при этом Axxon не загружается, то проблема может быть в поврежденных файлах Axxon,
   а не в SSD
...
3. Далее перейдите на вкладку - Exit и в разделе Boot Override выберите для загрузки накопитель - Kingston DataTraveler
   3.0 USB Device (Partition 1)
...
10. Пример:
    - После того как вы выбрали диск и указали его номер нажмите - Y для запуска процедуры восстановления заводского
      образа ОС.
...
1. В комплектации с сервером у вам идет USB накопитель IPDROM на нем имеется сертификат на изделие и руководство по
   эксплуатаци
...
1. В ОС Ubuntu 22.04 выполнить настройку video-dummy с графикой intel не представляется возможным. К некорректной работе
   GUI приводят конфигурации xorg.conf.
...
13. Для работы системы без подключённого монитора может потребоваться временное подключение монитора для инициализации
    графического интерфейса. Если постоянное подключение невозможно, можно использовать эмулятор монитора.

# 25. Проблемы с сетевым подключением LAN

## Рекомендации

1. Такая проблема может происходить по причине того, что в ОС Ubuntu по умолчанию не созданы профили подключения. Из-за
   этого может происходить конфликт настроек сети.
...
- Платы лучей/реле должны быть установлены в корпус, но либо не подключены, либо подключены неверно, т.к. в разделе USB
  устройств нет - SL USB IO
...
- Если платы там не отображаются, то значит неверно подключен шлейф от луча/реле к матплате. В таком случае необходимо
  при подключении к мат плате перевернуть коннектор на шлейфе на 180 градусов.
...
- Если любой из дисков в составе массива выйдет из строя, то данный диск автоматически встанет на место неисправного,
  без потери работоспособности всего RAID массива в целом.

## Для проверки работоспособности

1. Выберите любой диск входящий в состав массива и нажмите по нему правой кнопкой мыши. Затем выберите пункт - Make
   drive offline.
   Согласитесь с предупреждением которое выдаст контроллер.
   После этого выбранный диск в дереве дисков будет помечен красным.
   Контроллер запищит сиреной, а диск HotSpare должен автоматически встать на место диска который offline. Также у диска
   hotspare появится в скобках приписка - (Rebuilding). За ходом процесса можно наблюдать на вкладке - Dashboard в
   правом столбце (Background Operations)
   Чтобы выключить сирену на контроллере, нажмите по нему правой кнопкой мыши и выберите пункт - Silence Alarm.

2. Чтобы вернуть все назад как было, выберите диск в деревере который вы перевели в offline. Нажмите по нему правой
   кнопкой мыши, выберите пункт - Make Drive Online.
   После этого диск должен вернуться в изначальное состояние, а диск HotSpare также должен помечаться как HotSpare в
   дереве. Если этого не произошло автоматически, то нажмите по нему пр кнопкой мыши в выберите пункт - Assingn Global
   HotSpare.
...
# 31. Переустановка Astra Linux при помощи Clonzilla

## Рекомендации

- Перезагрузите сервер и во время загрузки нажимайте DEL для входа в BIOS.
- Затем нажмите F7 для перехода в режим - Advanced mode
- Далее перейдите на вкладку - Boot и в разделе Boot Override выберите пункт - USB Sandisk Partiton 1
- У вас должна загрузиться оболочка восстановления - Clonezilla
- Далее следуйте шагам приложенной инструкции.
...
# 32. При запуске ПК не происходит загрузка. При попытке выйти в BIOS зависает.

## Рекомендую попробовать сделать следующее:

- Обесточить ПК
- Открыть крышку корпуса и снять аккуратно круглую батарейку на материнской плате не менее чем на 1 мин.
- Затем вернуть батарейку на место и не закрывая крышку корпуса вынуть модули оперативной памяти из слотов и затем
  установить их на место. (Возможно потерялся контакт, из за чего ПК не грузится).
- Далее подключить ПК к питанию и не закрывая крышку корпуса включить его.
- Проверить на мат плате внизу 4 светодиода, сообщающих статус загрузки.
- При нормальной загрузке ПК Диодны должны загораться в следующем порядке - CPU=>DRAM=>VGA=>Boot (Затем Boot может
  оставаться гореть, или все диодны должны гаснуть).

# 33. При подаче основного питания загорается кнопка включения красным цветом на нажатия не реагирует запуск устройства не происходит

## Диагностика проблемы

- Откройте корпус устройства и проверьте что модули оперативной памяти корректно вставлены в слоты. Возможно что-то
  могло отойти. Попробуйте снять модули и затем попробовать поставить на место сначала 1 планку и попробовать запустить
  регистратор. Также проделайте со 2 планкой.
- Если регистратор запустится на одной из планок - то проблема скорее всего в неисправном модуле памяти.
- Если же проблема не уйдет, то установите обратно на места память.
- Полностью обесточьте регистратор отключив кабель питания из БП.
- Снимите с материнской платы маленькую круглую батарейку - не менее чем на 30 сек. (Это сбросит настройки BIOS)
- Затем верните батарейку на место и подайте питание на регистратор.
- Если регистратор включится и загрузится в BIOS или ОС, то проблема была в настройках BIOS.
- Если регистратор по-прежнему не включается - попробуйте запитать его от др блока питания.
- Если проблема так и не уйдет, то скорее всего вышла из строя материнская плата регистратора.
Сообщите по результату - помогли ли рекомендации.

# 34. Не запускается виндовс, восстановление системы через f12 не работает, ошибка синий экран

## Действия
1. Возможно проблема в переходнике или во флешке восстановления
2. Попробуйте вынут флешку из переходника на материнской плате, и подключить ее в USB порт сзади напрямую
3. Затем снова загрузитесь в оболочку восстановления и попробуйте восстановить образ
4. Вставьте флешку в любой др ПК с ОС Windows
5. После того как накопительно определится в ОС, нажмите по нему ПКМ в проводнике и выберите пункт - Свойства
6. Затем в открывшемся окне перейдите на вкладку - Сервис
7. Выберите пункт - Проверка на наличие ошибок
8. Дождитесь завершения проверки накопителя и затем снова подключите его в сервер и попробуйте восстановить образ ОС
9. Если какие-то проблемы при работе с флешкой, то проблема с ней
10. Проверьте запустить сервер без флешки